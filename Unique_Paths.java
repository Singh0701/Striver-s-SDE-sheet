...
// In test case 2, we are given a 1 x 6 matrix, hence we have a single row to traverse, and thus total path will be 1.
    
 
// Solution:

// Approach 1: Recursive approach, here we recall the unique paths function for each, a down step (m -1) and right step (n - 1) and check with the base condition if we reach at (1,1) means it's one of the possible paths so we increment the path count by returning 1.
...
    
// Approach 2: In the above approach, we'll use memoization with recursion and solve it in Polynomial-time.
