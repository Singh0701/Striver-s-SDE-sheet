...
// Approach 1: Generating all the possible dictionary permutations and returning the next one, if no next is present then the 1st permutation or just the reverse of the last one. This will have exponential space and time complexity, not gonna impement. Will move onto the optimal approach discussed below.
    
    
// Approach 2: In this approach we will use the peak point of an permutation to get the next one, that is all the DICTIONARY permutations will have a increasing peak followed by an decreasing one (Assume the last element as peak if no greater element is present) so with help of this first wewill traverse the array from backwards and see for a breakpoint that right after the peak value, and save that index of the breakpoint. 
// ***    
// Edge case: If in first iteration, we couldn't find any breakpoint that means the given permutation is the last one from it's dictionary orders. In this case just reverse (sort) the array and return it.    
// ***
// Now in another backwards traversal of the array will try to find the element which greater then the element at breakpoint index and swap both of them, now we can see that we have removed the existing breakpoint by swapping the elements so what that means is now we just need the smallest dictionary order in right half of the new peak value (that is the breakpoint index), that's basically a increasing order of elements in right half. (Reverse the array from breakpoint index + 1 to array length - 1).
